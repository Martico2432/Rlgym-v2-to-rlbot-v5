...
class MinimalistRelativeDefaultObs(ObsBuilder[AgentID, np.ndarray, GameState, Tuple[str, int]]):
    """
    ...
    def _build_obs(self, agent: AgentID, state: GameState, shared_info: Dict[str, Any]) -> np.ndarray:
        ...
        obs = [  # Global stuff
            ...
            [  # Partially observable variables
                ...
                car.can_flip,
                car.boost_amount * self.BOOST_COEF #? Only knows their boost, not others
        ...
        for (id_car, other_car), other_physics in zip(state.cars.items(), rel_cars):
            if id_car == agent:
                continue
